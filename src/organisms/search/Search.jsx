...
import { selectRoom, selectTab } from '@lib/matrix/action/navigation';
		...
		if (item.type === 'direct') {
			imageSrc =
				item.room
					...
					?.getAvatarUrl(mx.baseUrl, 24, 24, 'crop') || null;
		} else {
					...
					<ScrollView autoHide>
						<div className="search-dialog__content">
							{Array.isArray(result?.chunk) &&
								result.chunk.map(renderRoomSelector)}
						</div>
					</ScrollView>
				</div>
