...
import { selectRoom } from '@lib/matrix/action/navigation';
	...
	return (
		<div className="try-join-with-alias">
			{status.roomId === null && !status.isJoining && status.error === null && (
				<Button
					onClick={() => joinWithAlias()}
				>{`Try joining ${alias}`}</Button>
			)}
			{status.isJoining && (
				<>
					<Spinner size="small" />
...
function PublicRooms({ isOpen, searchTerm, onRequestClose }) {
	...
	async function searchRooms(viewMore) {
		...
		if (typeof inputHs !== 'string')
			inputHs = userId.slice(userId.indexOf(':') + 1);
		...
		try {
			...
			const totalRooms = viewMore
				? publicRooms.concat(result.chunk)
				: result.chunk;
			...
			return (
				<RoomTile
					...
					options={
						<>
							{isJoined && (
								<Button onClick={() => handleViewRoom(room.room_id)}>
									Open
								</Button>
							)}
							{!isJoined &&
