...
function genRoomIntro(mEvent, roomTimeline) {
	...
	const roomTopic = roomTimeline.room.currentState
	...
	return (
		<RoomIntro
			...
			desc={`This is the beginning of ${roomTimeline.room.name} room.${
				typeof roomTopic !== 'undefined' ? ` Topic: ${roomTopic}` : ''
			}`}
			time={
				mEvent
...
class TimelineScroll extends EventEmitter {
	constructor(target) {
		...
		if (target === null) {
			throw new Error(
				'Can not initialize TimelineScroll, target HTMLElement in null'
			);
		}
		this.scroll = target;
	...
	tryRestoringScroll() {
		...
		let scrollTop = 0;
		const ot = this.inTopHalf
			? this.topMsg?.offsetTop
			: this.bottomMsg?.offsetTop;
	...
	scrollToIndex(index, offset = 0) {
		...
		if (
			offsetTop > scrollInfo.top &&
			offsetTop < scrollInfo.top + scrollInfo.viewHeight
		)
			return;
	...
	_updateTopBottomMsg() {
		...
		this.topMsg =
			msgs[0]?.className === 'ph-msg' ? msgs[PLACEHOLDER_COUNT] : msgs[0];
		this.bottomMsg =
	...
	_calcMaxEvents(scrollInfo) {
		return (
			Math.round(scrollInfo.viewHeight / SMALLEST_MSG_HEIGHT) * PAGES_COUNT
		);
	}
...
function usePaginate(roomTimeline, readEventStore, forceUpdateLimit) {
	...
	useEffect(() => {
		const handleOnPagination = (backwards, loaded) => {
			...
			limit.setFrom(
				limit.calcNextFrom(backwards, roomTimeline.timeline.length)
			);
			setTimeout(() =>
				setInfo({
					backwards,
	...
	useEffect(() => {
		const sendReadReceipt = (event) => {
			...
			const unreadMsgIsLast =
				timeline[timeline.length - 2].getId() === readUpToEvent?.getId();
			if (unreadMsgIsLast) {
				roomTimeline.markAllAsRead();
			}
		};
...
function RoomViewContent({ eventId, roomTimeline }) {
	...
	useEffect(() => {
		...
		if (timeline.length > 0) {
			...
			if (timelineScroll.bottom < 16 && !roomTimeline.canPaginateForward()) {
				const readUpToId = roomTimeline.getReadUpToEventId();
				if (
					readEventStore.getItem()?.getId() === readUpToId ||
					readUpToId === null
				) {
					requestAnimationFrame(() => roomTimeline.markAllAsRead());
				}
	...
	const renderTimeline = () => {
		...
		if (
			roomTimeline.canPaginateForward() ||
			limit.getEndIndex() < timeline.length
		) {
