...
import { getPowerLabel, getUsernameOfRoomMember } from '@lib/util/matrixUtil';
...
function PeopleDrawer({ roomId }) {
	...
	useEffect(() => {
		...
		const updateMemberList = (event) => {
			...
			setMemberList(
				simplyfiMembers(
					getMembersWithMembership(membership).sort(AtoZ).sort(sortByPowerLevel)
				)
			);
		};
		searchRef.current.value = '';
			...
			<Header>
				<TitleWrapper>
					<Text variant="s1" primary>
						People
						<Text
							className="people-drawer__member-count"
							variant="b3"
						>{`${room.getJoinedMemberCount()} members`}</Text>
					</Text>
				</TitleWrapper>
				<IconButton
