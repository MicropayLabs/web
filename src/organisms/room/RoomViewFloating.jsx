...
function useJumpToEvent(roomTimeline) {
	...
	useEffect(() => {
		...
		if (
			!readEventId?.startsWith('~') &&
			!roomTimeline.hasEventInTimeline(readEventId)
		) {
		...
		return () => {
			roomTimeline.removeListener(
				cons.events.roomTimeline.MARKED_AS_READ,
				handleMarkAsRead
			);
			setEventId(null);
		};
...
function useScrollToBottom(roomTimeline) {
	...
	useEffect(() => {
		...
		return () =>
			roomTimeline.removeListener(
				cons.events.roomTimeline.AT_BOTTOM,
				handleAtBottom
			);
	}, [roomTimeline]);
...
function RoomViewFloating({ roomId, roomTimeline }) {
	const [isJumpToEvent, jumpToEvent, cancelJumpToEvent] =
		useJumpToEvent(roomTimeline);
	...
	return (
		<>
			<div
				className={`room-view__unread ${
					isJumpToEvent ? 'room-view__unread--open' : ''
				}`}
			>
			...
			<div
				className={`room-view__typing${
					typingMembers.size > 0 ? ' room-view__typing--open' : ''
				}`}
			>
			...
			<div
				className={`room-view__STB${isAtBottom ? '' : ' room-view__STB--open'}`}
			>
				<IconButton
