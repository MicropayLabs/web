...
import settings from '@lib/matrix/state/settings';
import { openEmojiBoard } from '@lib/matrix/action/navigation';
import navigation from '@lib/matrix/state/navigation';
import { bytesToSize, getEventCords } from '@lib/util/common';
import { getUsername } from '@lib/util/matrixUtil';
...
function RoomViewInput({ roomId, roomTimeline, viewEvent }) {
	...
	useEffect(() => {
		...
		return () => {
			settings.removeListener(
				cons.events.settings.MARKDOWN_TOGGLED,
				setIsMarkdown
			);
	...
	function uploadingProgress(myRoomId, { loaded, total }) {
		...
		uploadProgressRef.current.textContent = `Uploading: ${bytesToSize(
			loaded
		)}/${bytesToSize(total)} (${progressPer}%)`;
	...
	function replaceCmdWith(msg, cursor, replacement) {
		...
		if (replacement.length > 0)
			setCursorPosition(leadingInput.length + replacement.length);
	...
	useEffect(() => {
		...
		return () => {
			...
			roomsInput.removeListener(
				cons.events.roomsInput.ATTACHMENT_CANCELED,
				clearAttachment
			);
			roomsInput.removeListener(
				cons.events.roomsInput.FILE_UPLOADED,
				clearAttachment
			);
	...
	const canISend = roomTimeline.room.currentState.maySendMessage(
		mx.getUserId()
	);
			...
			<>
				...
				<div ref={inputBaseRef} className="room-input__input-container">
					{roomTimeline.isEncrypted() && (
						<RawIcon size="extra-small" src={ShieldIC} />
					)}
					<ScrollView autoHide>
		...
		return (
			<div className="room-attachment">
				...
				>
					...
					{fileType !== 'image' &&
						fileType !== 'video' &&
						fileType !== 'audio' && <RawIcon src={FileIC} />}
				</div>
				<div className="room-attachment__info">
					<Text variant="b1">{attachment.name}</Text>
					<Text variant="b3">
						<span ref={uploadProgressRef}>{`size: ${bytesToSize(
							attachment.size
						)}`}</span>
					</Text>
				</div>
