...
import { toggleMarkdown } from '@lib/matrix/action/settings';
import * as roomActions from '@lib/matrix/action/room';
import {
	openCreateRoom,
	openPublicRooms,
	openInviteUser,
...
import AsyncSearch from '@lib/util/AsyncSearch';
...
const commands = [
	...
	},
	{
		...
		isOptions: true,
		description:
			'Start direct message with user. Example: /startDM/@johndoe.matrix.org',
...
function renderSuggestions({ prefix, option, suggestions }, fireCmd) {
	function renderCmdSuggestions(cmdPrefix, cmds) {
		...
		return cmds.map((cmd) => (
			...
			>
				<Text variant="b2">{`${cmd.name}${
					cmd.isOptions ? cmdOptString : ''
				}`}</Text>
			</CmdItem>
		));
	}
...
function RoomViewCmdBar({ roomId, roomTimeline, viewEvent }) {
	...
	function processCmd(prefix, slug) {
		...
		if (prefix === ':') {
			if (searchTerm.length <= 3) {
				...
				else if (searchTerm.match(/^[-]?(p|P)$/))
					searchTerm = 'stuck_out_tongue';
	...
	function fireCmd(myCmd) {
		...
		if (myCmd.prefix === ':') {
			viewEvent.emit('cmd_fired', {
				replace: myCmd.result.mxc
					? `:${myCmd.result.shortcode}: `
					: myCmd.result.unicode,
			});
		}
		if (myCmd.prefix === '@') {
			viewEvent.emit('cmd_fired', {
