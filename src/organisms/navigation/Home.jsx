...
import { selectSpace, selectRoom } from '@lib/matrix/action/navigation';
import Postie from '@lib/util/Postie';
...
function Home({ spaceId }) {
	...
	if (spaceChildIds) {
		spaceIds = spaceChildIds
			.filter((roomId) => roomList.spaces.has(roomId))
			.sort(AtoZ);
		roomIds = spaceChildIds
			.filter((roomId) => roomList.rooms.has(roomId))
			.sort(AtoZ);
		directIds = spaceChildIds
			.filter((roomId) => roomList.directs.has(roomId))
			.sort(AtoZ);
	} else {
		spaceIds = [...roomList.spaces]
			.filter((roomId) => !roomList.roomIdToParents.has(roomId))
			.sort(AtoZ);
	...
	return (
		<>
			{spaceIds.length !== 0 && (
				<Text className="cat-header" variant="b3" weight="bold">
					Spaces
				</Text>
			)}
			{spaceIds.map((id) => (
