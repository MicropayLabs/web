...
import { selectSpace } from '@lib/matrix/action/navigation';
					...
					{spacePath.map((id, index) => {
						const noti =
							id !== cons.tabs.HOME && index < spacePath.length
								? getNotiExcept(
										id,
										index === spacePath.length - 1 ? null : spacePath[index + 1]
								  )
								: getHomeNotiExcept(
										index === spacePath.length - 1 ? null : spacePath[index + 1]
								  );
						...
						return (
							<React.Fragment key={id}>
								...
								<Button
									className={
										index === spacePath.length - 1
											? 'breadcrumb__btn--selected'
											: ''
									}
								...
								>
									<Text variant="b2">
										{id === cons.tabs.HOME
											? 'Home'
											: twemojify(mx.getRoom(id).name)}
									</Text>
									{noti !== null && (
										<NotificationBadge
											alert={noti.highlight !== 0}
											content={
												noti.total > 0 ? abbreviateNumber(noti.total) : null
											}
										/>
									)}
								</Button>
