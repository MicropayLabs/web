...
} from '@lib/matrix/action/settings';
import logout from '@lib/matrix/action/logout';
...
import PopupWindow, {
	PWContentSelector,
} from '../../molecules/popup-window/PopupWindow';
...
function GeneralSection() {
	return (
		<div className="settings-content">
			<SettingTile
				title=""
				content={<ProfileEditor userId={initMatrix.matrixClient.getUserId()} />}
			/>
		</div>
	);
}
	...
	return (
		<div className="settings-content">
			<SettingTile
				...
				content={
					<Text variant="b3">
						Use light or dark mode based on the system's settings.
					</Text>
				}
			/>
			{(() => {
			...
			<SettingTile
				...
				content={
					<Text variant="b3">
						Hide membership change messages from room timeline. (Join, Leave,
						Invite, Kick and Ban)
					</Text>
				}
			/>
...
function SecuritySection() {
	return (
		<div className="set-security settings-content">
			...
			/>
			<SettingTile
				title={`Device key: ${initMatrix.matrixClient
					.getDeviceEd25519Key()
					.match(/.{1,4}/g)
					.join(' ')}`}
				content={
