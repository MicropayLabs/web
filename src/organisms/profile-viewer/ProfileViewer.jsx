...
function ModerationTools({ roomId, userId }) {
	...
	const handleKick = (e) => {
		...
		roomActions.kick(
			roomId,
			userId,
			kickReason !== '' ? kickReason : undefined
		);
	};
...
function ProfileFooter({ roomId, userId, onRequestClose }) {
	...
	const [isUserIgnored, setIsUserIgnored] = useState(
		initMatrix.matrixClient.isUserIgnored(userId)
	);
	...
	const isInvitable =
		member?.membership !== 'join' && member?.membership !== 'ban';
...
function ProfileViewer() {
	const [isOpen, roomId, userId, closeDialog, handleAfterClose] =
		useToggleDialog();
