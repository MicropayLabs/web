...
function bindReplyToContent(roomId, reply, content) {
	...
	newContent.formatted_body =
		formattedReply + (content.formatted_body || content.body);
	return newContent;
}
...
function formatAndEmojifyText(room, text) {
	...
	Array.from(formattedText.matchAll(/\B:([\w-]+):\B/g))
		...
		.forEach((shortcodeMatch) => {
			...
			let tag;
			if (emoji.mxc) {
				tag = `<img data-mx-emoticon="" src="${emoji.mxc}" alt=":${emoji.shortcode}:" title=":${emoji.shortcode}:" height="32" />`;
			} else {
				tag = emoji.unicode;
			}
...
class RoomsInput extends EventEmitter {
	...
	async sendEditedMessage(roomId, mEvent, editedBody) {
		const isReply =
			typeof mEvent.getWireContent()['m.relates_to']?.['m.in_reply_to'] !==
			'undefined';
		...
		if (isReply) {
			...
			content.format = 'org.matrix.custom.html';
			content.formatted_body = `${fReplyHead}</mx-reply>${
