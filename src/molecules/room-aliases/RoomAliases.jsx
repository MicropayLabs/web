...
import { Debounce } from '@lib/util/common';
import { isRoomAliasAvailable } from '@lib/util/matrixUtil';
...
function RoomAliases({ roomId }) {
	...
	const [validate, setValidateToDefault, handleAliasChange] =
		useValidate(hsString);

	const canPublishAlias = room.currentState.maySendStateEvent(
	...
	useEffect(() => {
		...
		const loadLocalAliases = async () => {
			...
			try {
				...
				local = result.aliases.filter(
					(alias) => !aliases.published.includes(alias)
				);
			} catch {
				local = [];
			}
	...
	const renderAliasBtns = (alias) => {
		...
		if (deleteAlias?.alias === alias) {
			...
			return (
				<div className="room-aliases__item-btns">
					<Text variant="b2">
						<span
							style={{ color: isError ? 'var(--tc-danger-high' : 'inherit' }}
						>
							{deleteAlias.msg}
						</span>
					</Text>
		...
		return (
			<div className="room-aliases__item-btns">
				{canPublishAlias && !isMain && (
					<Button onClick={() => handleSetMainAlias(alias)} variant="primary">
						Set as Main
					</Button>
				)}
				{!isPublished && canPublishAlias && (
					<Button onClick={() => handlePublishAlias(alias)} variant="positive">
						Publish
					</Button>
				)}
				{isPublished && canPublishAlias && (
					<Button onClick={() => handleUnPublishAlias(alias)} variant="caution">
						Un-Publish
					</Button>
				)}
				<Button onClick={() => handleDeleteAlias(alias)} variant="danger">
					Delete
				</Button>
			</div>
		);
			...
			<React.Fragment key={`${alias}-wrapper`}>
				<div className="room-aliases__alias-item" key={alias}>
					<Checkbox
						...
						disabled={disabled}
						isActive={isActive}
						onToggle={() => handleAliasSelect(alias)}
					/>
					<Text>
						{alias}
	...
	return (
		<div className="room-aliases">
			<SettingTile
				...
				content={
					<Text variant="b3">{`Publish this room to the ${hsString}'s public room directory?`}</Text>
				}
				options={
					<Toggle
						isActive={isPublic}
			...
			<div className="room-aliases__content">
				...
				{aliases.published.length > 0 && !aliases.main && (
					<Text className="room-aliases__message">
						No Main address (select one from below)
					</Text>
				)}
				{aliases.published.map(renderAlias)}
				<Text className="room-aliases__message" variant="b3">
					Published addresses can be used by anyone on any server to join your
					room. To publish an address, it needs to be set as a local address
					first.
				</Text>
			</div>
			{isLocalVisible && (
				<div className="room-aliases__content">
					...
					<Text className="room-aliases__message" variant="b3">
						Set local addresses for this room so users can find this room
						through your homeserver.
					</Text>
					...
					<div className="room-aliases__input-status">
						...
						{validate.status === cons.status.ERROR && (
							<Text className="room-aliases__invalid" variant="b2">
								{validate.msg}
							</Text>
						)}
					</div>
