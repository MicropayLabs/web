...
import {
	...
	parseReply,
} from '@lib/util/matrixUtil';
import colorMXID from '@lib/util/colorMXID';
import { getEventCords } from '@lib/util/common';
import { redactEvent, sendReaction } from '@lib/matrix/action/roomTimeline';
import {
	openEmojiBoard,
	openProfileViewer,
	openReadReceipts,
...
import { sanitizeCustomHtml } from '@lib/util/sanitize';
...
const MessageAvatar = React.memo(({ roomId, mEvent, userId, username }) => {
	const avatarSrc = mEvent.sender.getAvatarUrl(
...
function genReactionMsg(userIds, reaction) {
	...
	userIds.forEach((userId, index) => {
		...
		else if (index === userIds.length - 1)
			msg = (
				<>
					{msg}
					{genLessContText(' and ')}
...
function MessageReaction({ reaction, count, users, isActive, onClick }) {
	return (
		<Tooltip
...
function Message({ mEvent, isBodyOnly, roomTimeline, focus, time }) {
