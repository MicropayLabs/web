...
function RoomProfile({ roomId }) {
	...
	avatarSrc = isDM
		? mx
				...
				?.getAvatarUrl(mx.baseUrl, 36, 36, 'crop')
		: avatarSrc;
	...
	const canChangeAvatar = currentState.maySendStateEvent(
		'm.room.avatar',
		userId
	);
	...
	const renderEditNameAndTopic = () => (
		<form className="room-profile__edit-form" onSubmit={handleOnSubmit}>
			...
			{(!canChangeName || !canChangeTopic) && (
				<Text variant="b3">{`You have permission to change room ${
					canChangeName ? 'name' : 'topic'
				} only.`}</Text>
			)}
